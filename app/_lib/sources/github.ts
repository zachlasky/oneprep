...
export async function fetchGithubPullRequests(login: string): Promise<GithubPullRequest[]> {
  const token = process.env.GITHUB_TOKEN;
  const query = `author:${login} type:pr`;
  const url = `${GITHUB_API}/search/issues?q=${encodeURIComponent(query)}&sort=updated&order=desc&per_page=20`;
